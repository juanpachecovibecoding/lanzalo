...
dash = dash.replace(
  ...
  '<a href={"/catalogo/" + user.uid} target="_blank" className="text-indigo-600 hover:text-indigo-700 font-bold text-sm bg-indigo-50 px-4 py-2 rounded-lg flex items-center gap-2">Ver Catálogo Público</a>' +
  '<button onClick={() => { navigator.clipboard.writeText(window.location.origin + "/catalogo/" + user.uid); alert("Link copiado"); }} className="text-slate-600 hover:text-slate-700 font-bold text-sm bg-slate-100 px-4 py-2 rounded-lg flex items-center gap-2">Copiar Link</button>' +
  '</div>'
);

// We should run replace globally for 'portal de reservas' just in case
